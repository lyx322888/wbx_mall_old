...
public class SearchHistoryDB extends SQLiteOpenHelper {
     ...
     * 查询全部搜索记录
     */
    public ArrayList<SearchInfo> queryAllHistory() {
        ArrayList<SearchInfo> historys = new ArrayList<>();
        //获取数据库对象
        ...
        for (cursor.moveToFirst(); !(cursor.isAfterLast()); cursor.moveToNext()) {
            ...
            searchInfo.setHistorys(history);
            searchInfo.setType(type);
            historys.add(searchInfo);
        }
        cursor.close();//关闭结果集
        db.close();//关闭数据库对象
        return historys;
